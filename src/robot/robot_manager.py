...
from src.sensors.warp.warp_sensor import WarpSensor
        ...
        if not self.use_warp:
            ...
            if self.warp_sensor_config is not None:
                ...
                if self.global_tensor_dict["CONST_WARP_MESH_ID_LIST"] is None:
                    ...
                    self.camera_orientation_tensor = torch.zeros(
                            (
                                self.num_envs,
                                self.warp_sensor_config.num_sensors,
                                3,
                    ...
                    self.global_tensor_dict["camera_orientation"] = self.camera_orientation_tensor
                    ...
                    if self.warp_sensor_config.face_mesh_camera:
                        self.face_mesh_tensor = torch.zeros(
                            (
                                self.num_envs,
                                self.warp_sensor_config.num_sensors,
                                self.warp_sensor_config.height,
                        ...
                        self.global_tensor_dict["face_mesh_pixels"] = self.face_mesh_tensor
                    if self.warp_sensor_config.occupancy_map_lidar:
                        self.occupancy_map_lidar_tensor = torch.zeros(
                            (
                                self.num_envs,
                                self.warp_sensor_config.word_map_grid_size,
                                self.warp_sensor_config.word_map_grid_size,
                                self.warp_sensor_config.word_map_grid_size,
                            ),
                            device=self.device,
                        ...
                        self.global_tensor_dict["occupancy_map"] = self.occupancy_map_lidar_tensor
                    ...
                    self.warp_sensor.update(self.actions)
