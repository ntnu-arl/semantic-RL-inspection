...
class BaseSensor(ABC):
    ...
    def init_tensors(self, global_tensor_dict):
        ...
        if self.cfg.sensor_type == "lidar" or self.cfg.sensor_type == "camera":
            ...
            self.camera_orientation = global_tensor_dict["camera_orientation"]
            if self.cfg.occupancy_map_lidar:
                self.occupancy_map = global_tensor_dict["occupancy_map"]
            else:
                self.occupancy_map = None
            
            if self.cfg.face_mesh_camera:
                self.segmentation_pixels = global_tensor_dict["segmentation_pixels"]
                self.face_mesh_pixels = global_tensor_dict["face_mesh_pixels"]
            elif self.cfg.segmentation_camera:
                self.segmentation_pixels = global_tensor_dict["segmentation_pixels"]
                self.face_mesh_pixels = None
            else:
